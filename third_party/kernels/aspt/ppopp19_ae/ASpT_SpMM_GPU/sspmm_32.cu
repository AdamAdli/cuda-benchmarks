...
__global__ void print_csr_kernel(
  int *row_ptrs,
  int *col_idxs,
  FTYPE *values,
  int n,
  ...
  for (int i = 0; i < n; i++) {
    for (int p = row_ptrs[i]; p < row_ptrs[i+1]; p++) {
      printf("(%d, %d): %f\n", i, col_idxs[p], values[p]);
      if (++ele_printed >= cap) return;
    }
  }
}
...
void spmv_kernel32_sparse_v2(int sc, int *csr_v, int *csr_e, FTYPE *csr_ev, int *mcsr_cnt, int *mcsr_e, int *mcsr_list,
    ...
    //print_csr_kernel(csr_v, csr_e, csr_ev, 100, 10);
...
__global__
//__launch_bounds__(BSIZE, 2048/BSIZE)
void spmv_kernel32_sparse_v2l(int sc, int *csr_v, int *csr_e, FTYPE *csr_ev, int *mcsr_cnt, int *mcsr_e, int *mcsr_list,
                              FTYPE *vin, FTYPE *vout) {
    int idx = (blockIdx.x * SBF) + (threadIdx.x >> 5);// + (threadIdx.x>>(LOG_MFACTOR));
    int lane = (threadIdx.x & (MFACTOR - 1));
    int offset = lane;
...
__global__
//__launch_bounds__(BSIZE, 2048/BSIZE)
void spmv_kernel32_sparse_v2h(int sc, int *csr_v, int *csr_e, FTYPE *csr_ev, int *mcsr_cnt, int *mcsr_e, int *mcsr_list,
                              FTYPE *vin, FTYPE *vout, int *special, int *special2) {
...
__global__
//__launch_bounds__(BSIZE, 2048/BSIZE)
void spmv_kernel32_ssparse(int sc, int *csr_v, int *csr_e, FTYPE *csr_ev, int *mcsr_cnt, int *mcsr_e, int *mcsr_list,
                           FTYPE *vin, FTYPE *vout) {
    int idx = (blockIdx.x * SBF) + (threadIdx.x >> 5);// + (threadIdx.x>>(LOG_MFACTOR));
    int lane = (threadIdx.x & (MFACTOR - 1));
    int offset = lane;
...
__global__
//__launch_bounds__(BSIZE, 2048/BSIZE)
void spmv_kernel32_dense_v2(int sc, int *csr_v, int *csr_e, FTYPE *csr_ev, int *mcsr_cnt, int *mcsr_e, int *mcsr_list,
                            FTYPE *vin, FTYPE *vout, int *baddr, int *saddr) {
    int lane = (threadIdx.x & (MFACTOR - 1));
    int offset = lane;
    int loop, i, j;
...
__global__ void
key2_marking(int *csr_pivot, int *key, STYPE *key2, int *val, int *mcsr_cnt, int *mcsr_list, int *baddr, int *saddr,
             int *mcsr_chk) {
    if (mcsr_chk[blockIdx.x] == 0) return;
    int lb = csr_pivot[blockIdx.x] + THRESHOLD - 1;
    int ub = csr_pivot[blockIdx.x + 1];
...
void process() {
    ...
    printf("CSR...\n");
    print_csr(csr_v, csr_e0, csr_ev0, nr, 10);

    printf("GOLD...\n");
    for (i = 0; i < 10; i++) {
      printf("(%d, %d): %f\n", gold_temp_v[i].row, gold_temp_v[i].col, gold_temp_v[i].val);
    }
    ...
    cudaMemset(_vout, 1, sizeof(FTYPE) * nr * sc);
    for (i = 0; i < nc * sc; i++) {
...
#define MCSR_CNT_TBSIZE (1024)
    ...
    if (d_flag[0] == 0) {
        ...
        key2_marking<<<npanel, MCSR_CNT_TBSIZE>>>(_csr_pivot, _key, _key2, _val, _mcsr_cnt, _mcsr_list, _baddr, _saddr,
                                                  _mcsr_chk);
    ...
    if (ne / nc < 6 && vari < 40) {
      printf("ne / nc < 6 && vari < 40\n");
        ...
        for (int ik = 0; ik < ITER; ik++) {
            // kernel fun
            spmv_kernel32_ssparse<<<ss_gridsize, ss_blocksize, 0, stream1>>>(sc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt,
                                                                             _mcsr_e, _mcsr_list, _vin, _vout);
            CUDA_CHECK_ERROR;
        }
      ...
      printf("vari < 200\n");
        dim3 one(1,1,1);
        print_csr_kernel<<<one, one, 0, stream1>>>(_csr_v, _csr_e, _csr_ev, nr, 10);
        CUDA_CHECK_ERROR;
        cudaDeviceSynchronize();
        CUDA_CHECK_ERROR;
        ...
        for (int ik = 0; ik < ITER; ik++) {
            // kernel fun
            spmv_kernel32_sparse_v2<<<s_gridsize, s_blocksize, 0, stream1>>>(sc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt,
                                                                             _mcsr_e, _mcsr_list, _vin, _vout);
            CUDA_CHECK_ERROR;
            if (num_dense) {
                spmv_kernel32_dense_v2<<<d_gridsize, d_blocksize, 0, stream2>>>(sc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt,
                                                                                _mcsr_e, _mcsr_list, _vin, _vout, _baddr,
                                                                                _saddr);
                CUDA_CHECK_ERROR;
            }
        }
      ...
      printf("else ...\n");
        ...
        for (int ik = 0; ik < ITER; ik++) {
            // kernel fun
            spmv_kernel32_sparse_v2l<<<s_gridsize, s_blocksize, 0, stream1>>>(sc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt,
                                                                              _mcsr_e, _mcsr_list, _vin, _vout);
            CUDA_CHECK_ERROR;
            spmv_kernel32_sparse_v2h<<<s_gridsizeh, s_blocksizeh, 0, stream3>>>(sc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt,
                                                                                _mcsr_e, _mcsr_list, _vin, _vout,
                                                                                _special, _special2);
            CUDA_CHECK_ERROR;
            spmv_kernel32_dense_v2<<<d_gridsize, d_blocksize, 0, stream2>>>(sc, _csr_v, _csr_e, _csr_ev, _mcsr_cnt,
...
//      fprintf(stdout, "num_diff : %d\n", num_diff);
    printf("%f,", (double) num_diff / (nr * sc) * 100);
    printf("%d/%d\n", num_diff, nr*sc);
