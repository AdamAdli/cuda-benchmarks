...
int dense_multiply(cudaStream_t &stream, cudaEvent_t &start, cudaEvent_t &stop, const Dense &A, const Dense &B,
                   const Dense &C) {
...
int
sgk_multiply(cudaStream_t &stream, cudaEvent_t &start, cudaEvent_t &stop, const CSR<float> &A_h, const CSR<float> &A,
    ...
    for (auto &codelet: codelets) {
        ...
        for (int i = 0; i < codelet.row_offsets.size(); i += rows_per_block) {
            for (int j = 0; j < codelet.col_offsets.size(); j += cols_per_block) {
                ...
                for (int ii = 0; ii < rows_in_block; ii++) {
                    curr_batch.rows[row_offset + ii] = codelet.row_offsets[i + ii];
                }
                for (int jj = 0; jj < cols_in_block; jj++) {
                    curr_batch.col_pattern[col_offset + jj] = codelet.col_offsets[j + jj];
                }
...
CSR<float> gen_csr(size_t m, size_t n, const std::vector <codelet_t> &codelets) {
    ...
    int nnz = 0;
    for (auto &row: rows) nnz += row.size();
...
CSR<float> read_coo_blocked(const std::string &path, std::vector <CodeletMultiply::Block> &blocks,
    ...
    if (token != "%csr") {
        std::cout << "csr token mismatch" << std::endl;
        exit(1);
    }
