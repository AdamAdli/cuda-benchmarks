...
#define CHECK_CUDA(func)                                                  \
    {                                                                     \
        ...
        {                                                                 \
            py::print("CUDA API failed at line %d with error: %s (%d)\n", \
                   __LINE__, cudaGetErrorString(status), (int) status);   \
            return EXIT_FAILURE;                                          \
        }                                                                 \
    }
...
#define CHECK_CUSPARSE(func)                                                  \
    {                                                                         \
        ...
        {                                                                     \
            py::print("CUSPARSE API failed at line %d with error: %s (%d)\n", \
                   __LINE__, cusparseGetErrorString(status), (int) status);   \
            return EXIT_FAILURE;                                              \
        }                                                                     \
    }
...
float test_sgk_spmm(
        int m, int n, int k, int nonzeros,
        py::array_t<float> A_value, py::array_t<int> A_row_offsets, py::array_t<int> A_col_indices,
        py::array_t<float> B_values)
{
  ...
  std::sort(swizzle_staging.begin(), swizzle_staging.end(),
            [&h_row_offsets](int idx_a, int idx_b) {
                int length_a = h_row_offsets[idx_a + 1] - h_row_offsets[idx_a];
                int length_b = h_row_offsets[idx_b + 1] - h_row_offsets[idx_b];
                return length_a > length_b;
            });

  // Copy the ordered row indices to the output.
  std::memcpy(h_row_offsets, swizzle_staging.data(), sizeof(int) * m);

  return _test_sgk_spmm(
      m, n, k, nonzeros,
      h_values, swizzle_staging.data(), h_row_offsets, h_col_indices,
      h_dense_matrix
  );
}
    ...
    py::array_t<float> B_values)
{
    ...
    return _test_sgk_spmm(
        m, n, k, nonzeros,
        h_values, h_row_indices, h_row_offsets, h_col_indices,
        h_dense_matrix
    );
}
...
PYBIND11_MODULE(kernel_python_bindings, m)
{
    ...
    m.def("sgk", &test_sgk_spmm,"the function returning the RT of sgk");
    m.def("sgk_custom_row_order", &test_sgk_spmm_custom_row_order, "the function returning the RT of sgk, with custom row swizzle");
}